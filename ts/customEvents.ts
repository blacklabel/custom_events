	...
	Object.entries(handlers).forEach(([eventName, handler]) => {
	  if (handler) {
		...
		if (!(el as any)._eventBound[eventName]) {
		  el.element.addEventListener(eventName, handler);
  ...
  export default function ObjectEventsPlugin(H: typeof Highcharts) {
	H.addEvent(H.Chart, "load", function (this: Chart) {
	  ...
	  chart.axes.forEach(axis => {
		...
		bindElementEvents(axis.title, axis.options.title?.events);

		// If you want to enable labelGroup events, you may need to extend the type for axis.options.labels as well:
		// const axisLabelOptions = axis.options.labels as { events?: ElementEvents } | undefined;
		// bindElementEvents(axis.labelGroup, axisLabelOptions?.events);
	...
	// 	  if (point.graphic) bindElementEvents(point.graphic, point.options?.events);
	// 	  if (point.dataLabel) bindElementEvents(point.dataLabel, series.options.dataLabels?.events);
	// 	});
	//   });
  
	//   // Legend
	//   bindElementEvents(chart.legend?.group, chart.options.legend?.events);
	});
  }
