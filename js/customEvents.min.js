...
return false;});}else{addEvent(SVGelem.element,event,function(e){e.stopPropagation();if(isSeries){var chart=eventObject.chart,normalizedEvent=chart.pointer.normalize(e);if(!eventObject.directTouch){elemObj=eventObject.searchPoint(normalizedEvent,eventObject.kdDimensions===1);}
e.point=elemObj;}
...
if(elemObj&&elemObj.textStr){elemObj.value=elemObj.textStr;}
if(isPoint&&event==='click'&&elemObj.series&&elemObj.series.options&&elemObj.series.options.allowPointSelect){var defaultFunction=function(event){if(elemObj.select){elemObj.select(null,e.ctrlKey||e.metaKey||e.shiftKey);}};HC.fireEvent(elemObj,event,e,defaultFunction);}
if(elemObj&&elemObj.drilldown){elemObj.doDrilldown(undefined,undefined,e);}else if(events&&events[event]){events[event].call(elemObj,e);}
return false;});}
...
if(stackLabels&&stackLabels.enabled){eventsPoint=stackLabels.events;elementPoint=this.stacks||this.stacking.stacks;eventsStackLabel=stackLabels.events;elementStackLabel=this.stackTotalGroup||this.stacking.stackTotalGroup;}
...
return{events:events,element:element,eventsPoint:eventsPoint,elementPoint:elementPoint,eventObject:this,isSeries:true};},renderItem:function(item){return{events:this.options.itemEvents,element:item.legendGroup||item.legendItem.group,eventObject:item};},drawCrosshair:function(){var cross=this.cross,crosshairOptions=this.options.crosshair;if(cross){cross.css({'pointer-events':'auto'});}
